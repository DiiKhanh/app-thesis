...
PRETRAIN_MODEL_FILEPATH = "../../pretrain_models/resnet50-19c8e357.pth"
...
def load_challenge_models(model_folder, verbose):
    model_folder = model_folder.lower()
    filename = hf_hub_download(repo_id=model_folder, filename="resnet/models.sav")
    model = joblib.load(filename)
    file_path_eeg = hf_hub_download(repo_id=model_folder, filename="resnet/eeg/checkpoint_best.pth")
    if USE_TORCH:
        print("Load model...")
...
class TorchvisionModel(nn.Module):
    def __init__(
        ...
        self.additional_layer = nn.Linear(additional_features, additional_features) if additional_features > 0 else None

        if "resnet" in self.model_name:
            self.model.conv1 = nn.Conv2d(channel_size, 64, kernel_size=7, stride=2, padding=3, bias=False)
            self.model.fc = nn.Identity()  # Ta xử lý thủ công trong forward

            self.resnet_feature_extractor = self.model
            num_features = 512 if "18" in self.model_name or "34" in self.model_name else 2048
            self.classifier = nn.Linear(num_features + additional_features, self.num_classes) \
                if additional_features > 0 else nn.Linear(num_features, self.num_classes)
    ...
    def forward(self, x, additional_x=None):
        if "efficientnet" in self.model_name:
            ...
            x = self.resnet_feature_extractor.conv1(x)
            x = self.resnet_feature_extractor.bn1(x)
            x = self.resnet_feature_extractor.relu(x)
            x = self.resnet_feature_extractor.maxpool(x)

            x = self.resnet_feature_extractor.layer1(x)
            x = self.resnet_feature_extractor.layer2(x)
            x = self.resnet_feature_extractor.layer3(x)
            x = self.resnet_feature_extractor.layer4(x)
            ...
            raise NotImplementedError(f"Model {self.model_name} not supported in forward")
