...
VOTING_POS_MAJORITY_THRESHOLD = 0.4
INFUSE_STATIC_FEATURES = True
ONLY_EEG_TORCH = True
ONLY_EEG_ROCKET = False
PARAMS_DEVICE = {"num_workers": 5}  # os.cpu_count()}
...
USE_BEST_MODEL = True  # If this is true, the best model is used for prediction, otherwise the last model is used
...
def load_challenge_models(model_folder, verbose):
    model_folder = model_folder.lower()
    filename = hf_hub_download(repo_id=model_folder, filename="convnext/models.sav")
    model = joblib.load(filename)
    file_path_eeg = hf_hub_download(repo_id=model_folder, filename="convnext/eeg/checkpoint_best.pth")
    if USE_TORCH:
        print("Load model...")
...
class TorchvisionModel(torch.nn.Module):
    def __init__(
        ...
        if "resnet" in model_name.lower():
            ...
            # Modify the first convolutional layer (stem) in ConvNeXt
            # self.model.features is a Sequential, features[0] is the stem (Sequential), features[0][0] is the Conv2d
            original_stem_conv = self.model.features[0][0]
            self.model.features[0][0] = torch.nn.Conv2d(
                ...
                padding=original_stem_conv.padding, # Typically 0 for ConvNeXt stem
                bias=original_stem_conv.bias is not None
            )
            
            # The classifier in ConvNeXt is typically: (LayerNorm2d, Flatten, Linear)
            # So the Linear layer is at index 2
            num_img_features = self.model.classifier[2].in_features

            if additional_features > 0:
                # The new classifier will take flattened image features + additional features.
                # This input will be 2D (batch_size, total_features), so a 1D LayerNorm is appropriate.
                self.model.classifier = nn.Sequential(
                    # The original LayerNorm2d (self.model.classifier[0]) and Flatten (self.model.classifier[1])
                    # will be effectively handled by how we process features in the forward pass
                    # before feeding to this new classifier.
                    nn.LayerNorm(num_img_features + additional_features, eps=1e-6),
                    nn.Linear(num_img_features + additional_features, self.num_classes)
                )
            else:
                self.model.classifier = nn.Sequential(
                    nn.LayerNorm(num_img_features, eps=1e-6),
                    nn.Linear(num_img_features, self.num_classes)
                )

        elif "efficientnet" in model_name.lower():
            # Thay đổi lớp đầu vào để phù hợp với số kênh đầu vào
            self.model.features[0] = nn.Conv2d(channel_size, 24, kernel_size=7, stride=2, padding=3, bias=False)
            
            # Lưu lại feature extractor và tạo classifier mới
            self.features_extractor = self.model.features
            
            # Lấy số đặc trưng đầu ra từ feature extractor
            num_features = self.model.classifier[1].in_features if hasattr(self.model, 'classifier') and isinstance(self.model.classifier, nn.Sequential) else self.model.classifier.in_features
            
            # Tạo classifier mới để kết hợp với đặc trưng bổ sung
            if additional_features > 0:
                self.classifier = nn.Linear(num_features + additional_features, self.num_classes)
            else:
    ...
    def forward(self, x, additional_x=None, classify=True):
        if "efficientnet" in self.model_name.lower() and self.additional_features > 0:
            ...
            combined_features = torch.cat([features, additional_x], dim=1)
            output = self.classifier(combined_features)
            return output

        elif "convnext" in self.model_name.lower():
            # Get features from the backbone
            img_features = self.model.features(x) # Output: (B, C_orig, H', W')
            img_features = self.model.avgpool(img_features) # Output: (B, C_orig, 1, 1)
            
            # The original ConvNeXt classifier applies LayerNorm2d then Flatten.
            # If we are adding features, we need to flatten the image features first.
            img_features_flat = torch.flatten(img_features, 1) # Output: (B, C_orig)

            if self.additional_features > 0:
                assert additional_x is not None, "additional_x cannot be None for ConvNeXt with additional_features"
                assert additional_x.shape[1] == self.additional_features, "Wrong shape of additional features for ConvNeXt"
                
                processed_additional_x = self.additional_layer(additional_x)
                combined_features = torch.cat([img_features_flat, processed_additional_x], dim=1)
                # The self.model.classifier is already the new Sequential(LayerNorm1D, Linear)
                output = self.model.classifier(combined_features)
            else:
                # If no additional features, use the modified classifier (LayerNorm1D, Linear)
                # which expects flattened input.
                output = self.model.classifier(img_features_flat)
            return output

        elif self.additional_features > 0:
            assert additional_x is not None, "additional_x cannot be None when using additional_features"
            assert additional_x.shape[1] == self.additional_features, "Wrong shape of additional features"

            # Với các model như resnet, densenet
            features = self.model.features(x)
            if len(features.shape) == 4:
                features = F.adaptive_avg_pool2d(features, (1, 1)).squeeze(-1).squeeze(-1)

            additional_x = self.additional_layer(additional_x)
            features = torch.cat([features, additional_x], dim=1)
