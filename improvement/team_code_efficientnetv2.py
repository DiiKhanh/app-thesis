...
PRETRAIN_MODEL_FILEPATH = "../../pretrain_models/efficientnet_v2_s-dd5fe13b.pth"
...
def load_challenge_models(model_folder, verbose):
    model_folder = model_folder.lower()
    filename = hf_hub_download(repo_id=model_folder, filename="efficientnet/models.sav")
    model = joblib.load(filename)
    file_path_eeg = hf_hub_download(repo_id=model_folder, filename="efficientnet/eeg/checkpoint_best.pth")
    if USE_TORCH:
        print("Load model...")
...
class TorchvisionModel(torch.nn.Module):
    ...
    def forward(self, x, additional_x=None, classify=True):
        if "efficientnet" in self.model_name.lower() and self.additional_features > 0:
            ...
            # Xử lý cho các mô hình khác có đặc trưng bổ sung
            assert additional_x is not None, "additional_x cannot be None when using additional_features"
            assert additional_x.shape[1] == self.additional_features, "Wrong shape of additional features"
            
            features = self.model.features(x)  # Sử dụng phần convolutional
            if len(features.shape) == 4:
                features = F.adaptive_avg_pool2d(features, (1, 1)).squeeze(-1).squeeze(-1)
                
            additional_x = self.additional_layer(additional_x)
            features = torch.cat([features, additional_x], dim=1)  # Kết hợp theo chiều feature
            x = self.model.classifier(features)  # Sử dụng classifier
        else:
            # Trường hợp không có đặc trưng bổ sung
